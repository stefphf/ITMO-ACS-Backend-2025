...
export default class RouteController {
    ...
    async create(req: Request, res: Response) {
        ...
        // Проверяем пользователя, если user_id передан
        if (userId !== undefined) {
        ...
        const route = routeRepo.create(req.body);
        // @ts-ignore
        const result = (await routeRepo.save(route)) as Route;

        await rabbitMQ.publish("route_events", "route.created", {
            id: result.id,
            title: result.title,
