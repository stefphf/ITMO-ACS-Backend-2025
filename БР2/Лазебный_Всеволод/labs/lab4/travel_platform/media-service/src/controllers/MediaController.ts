...
export default class MediaController {
    ...
    async create(req: Request, res: Response) {
        // Проверяем существование маршрута в Content Service
        const { route_id } = req.body;
        try {
            const contentServiceUrl = process.env.CONTENT_SERVICE_URL || 'http://localhost:3002';
            const response = await fetch(`${contentServiceUrl}/api/routes/${route_id}`, {
                headers: {
            ...
            return res.status(500).json({ message: 'Failed to verify route' });
        }
