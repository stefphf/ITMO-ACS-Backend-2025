...
export class JobOfferController {
    ...
    constructor() {
        this.repository = jobDataSource.getRepository(JobOffer)
        this.companyRepository = jobDataSource.getRepository(Company)
        this.jobCategoryRepository = jobDataSource.getRepository(JobCategory)
        this.skillRepository = jobDataSource.getRepository(Skill)
        this.jobOfferRequiredSkillRepository = jobDataSource.getRepository(JobOfferRequiredSkill)
    }

    async getAll(): Promise<JobOffer[]> {
        try {
            return await this.repository.find({
                relations: ['company', 'jobCategory', 'requiredSkills', 'requiredSkills.skill']
            })
        } catch (error) {
    ...
    async delete(id: string): Promise<void> {
        try {
            ...
            if (deleteResult.affected === 0) {
                throw new HttpError(404, "Job offer not found for deletion")
            }
        } catch (error) {
            if (error instanceof HttpError) throw error
    ...
    async getByCategoryId(categoryIdParam: string): Promise<JobOffer[]> {
        try {
            ...
            if (isNaN(categoryId)) throw new HttpError(400, "Invalid category ID format")
    ...
    async getByUserId(userIdParam: string): Promise<JobOffer[]> {
        try {
            ...
            if (error instanceof HttpError) throw error
            console.error("Error in JobOfferController.getByUserId:", error)
            throw new HttpError(500, "Error fetching job offers by user ID")
        }
    }
} 