...
export const queryValidationMiddleware = <T extends object>(
    dtoClass: ClassConstructor<T>,
    ...
    return async (req: Request, res: Response, next: NextFunction) => {
        ...
        for (const [key, value] of Object.entries(req.query)) {
            if (value !== undefined && value !== '') {
                ...
                queryData[key] = isNaN(numValue) ? value : numValue;
            }
        }

        const dtoInstance = plainToInstance(dtoClass, queryData);
        const errors: ClassValidatorError[] = await validate(dtoInstance, { skipMissingProperties });
