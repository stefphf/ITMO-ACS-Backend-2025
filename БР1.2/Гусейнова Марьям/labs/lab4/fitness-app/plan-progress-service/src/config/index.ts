...
const appConfig: AppConfig = {
  ...
  db: {
    ...
    database: process.env.DB_DATABASE || 'fitness_plans_progress_db', // Используем DB_DATABASE, переданный для plan-progress-service
  },
...
if (!appConfig.db.password) {
  console.error('DB_PASSWORD is not set in environment variables for Plan & Progress Service. Exiting.');
  process.exit(1);
}
if (!appConfig.db.database) {
  console.error('DB_DATABASE is not set in environment variables for Plan & Progress Service. Exiting.');
  process.exit(1);
}
