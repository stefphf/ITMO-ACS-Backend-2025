...
let WorkExperiencesService = class WorkExperiencesService {
    ...
    workExperienceGetById(id) {
        const workExperience = this.prisma.workExperience.findUnique({
            where: { id },
        });
        if (!workExperience) {
            throw new common_1.NotFoundException('workExperience not found');
        }
        return workExperience;
    }
    workExperienceCreate(dto) {
        return this.prisma.workExperience.create({
            data: dto,
        });
    }
    workExperienceUpdate(id, dto) {
        return this.prisma.workExperience.update({
            where: { id },
            data: dto,
        });
    }
    workExperienceDelete(id) {
