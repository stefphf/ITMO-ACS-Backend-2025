...
export interface ITrainingService {
   ...
   */
  createFreeTraining(trainingData: CreateFreeTrainingDto): Promise<FreeTrainingDto>;
   ...
   */
  updateFreeTraining(id: number, trainingData: UpdateFreeTrainingDto): Promise<FreeTrainingDto>;
   ...
   */
  addSeriesToFreeTraining(training_id: number, seriesData: CreateSeriesDto): Promise<SeriesDto>;

  /**
   * Создать новую серию
   * @param seriesData Данные серии
   */
  addSeriesToQualificationTraining(seriesData: CreateSeriesDto): Promise<SeriesDto>;
   ...
   * @param athleteId ID спортсмена
   */
  getQualificationTrainingsByAthleteId(athleteId: number): Promise<QualificationTrainingDto[]>;
}
