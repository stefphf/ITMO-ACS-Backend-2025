...
import { NoteDto, CreateNoteDto, UpdateNoteDto } from '../dtos/notes.dto';

@Service()
export class NotesService {
  async getAllNotes(userId: number): Promise<NoteDto[]> {
    const notes = await dataSource.getRepository(NoteEntity).find({ where: { user_id: userId } });
  ...
  async getNoteById(id: number, userId: number): Promise<NoteDto> {
    const note = await dataSource
      ...
      .findOne({ where: { id, user_id: userId } });
    if (!note) {
  ...
  async updateNote(id: number, dto: UpdateNoteDto, userId: number): Promise<NoteDto> {
    const note = await dataSource
      .getRepository(NoteEntity)
      .findOne({ where: { id, user_id: userId } });
    if (!note) {
  ...
  async deleteNote(id: number, userId: number): Promise<void> {
    const result = await dataSource.getRepository(NoteEntity).delete({ id, user_id: userId });
  ...
  private mapNoteToDto(note: NoteEntity): NoteDto {
    return {
      ...
      updatedAt: note.edited_at || note.created_at,
    };
  }
}
