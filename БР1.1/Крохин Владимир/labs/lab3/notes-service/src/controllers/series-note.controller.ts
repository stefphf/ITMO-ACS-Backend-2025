...
import { CreateSeriesNoteDto, SeriesNoteDto } from '../dtos/note.dto';
...
export class SeriesNoteController {
  ...
  constructor() {
    ...
    this.noteService = new NoteService(noteRepository, undefined, seriesNoteRepository);
  }
  ...
  @ResponseSchema(SeriesNoteDto)
  async createSeriesNote(@Body() dto: CreateSeriesNoteDto): Promise<SeriesNoteDto> {
    try {
  ...
  @ResponseSchema(SeriesNoteDto, { isArray: true })
  async getNotesBySeriesId(@Param('seriesId') seriesId: number): Promise<SeriesNoteDto[]> {
    try {
  ...
  @ResponseSchema(SeriesNoteDto, { isArray: true })
  async getSeriesNotesByUserId(@Param('userId') userId: number): Promise<SeriesNoteDto[]> {
    try {
      ...
      throw new BadRequestError('Не удалось получить заметки к сериям для пользователя');
    }
  }
  ...
  @ResponseSchema(SeriesNoteDto, { isArray: true })
  async getCurrentUserSeriesNotes(@Req() request: RequestWithUser): Promise<SeriesNoteDto[]> {
    try {
      const userId = request.user.id;
      return await this.noteService.getSeriesNotesByUserId(userId);
    } catch (error) {
      console.error('Ошибка при получении заметок к сериям текущего пользователя:', error);
      throw new BadRequestError('Не удалось получить заметки к сериям текущего пользователя');
    }
  }
}
