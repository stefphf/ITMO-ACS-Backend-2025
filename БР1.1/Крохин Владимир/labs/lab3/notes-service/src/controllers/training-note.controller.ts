...
import { CreateTrainingNoteDto, TrainingNoteDto } from '../dtos/note.dto';
...
export class TrainingNoteController {
  ...
  async getAllTrainingNotes(): Promise<TrainingNoteDto[]> {
    try {
      ...
      throw new BadRequestError('Не удалось получить список заметок к тренировкам');
    }
  }
  ...
  @ResponseSchema(TrainingNoteDto)
  async createTrainingNote(@Body() dto: CreateTrainingNoteDto): Promise<TrainingNoteDto> {
    try {
  ...
  @ResponseSchema(TrainingNoteDto, { isArray: true })
  async getNotesByTrainingId(@Param('trainingId') trainingId: number): Promise<TrainingNoteDto[]> {
    try {
  ...
  @ResponseSchema(TrainingNoteDto, { isArray: true })
  async getTrainingNotesByUserId(@Param('userId') userId: number): Promise<TrainingNoteDto[]> {
    try {
      ...
      throw new BadRequestError('Не удалось получить заметки к тренировкам для пользователя');
    }
  }
  ...
  @ResponseSchema(TrainingNoteDto, { isArray: true })
  async getCurrentUserTrainingNotes(@Req() request: RequestWithUser): Promise<TrainingNoteDto[]> {
    try {
      const userId = request.user.id;
      return await this.noteService.getTrainingNotesByUserId(userId);
    } catch (error) {
      console.error('Ошибка при получении заметок к тренировкам текущего пользователя:', error);
      throw new BadRequestError('Не удалось получить заметки к тренировкам текущего пользователя');
    }
  }
}
