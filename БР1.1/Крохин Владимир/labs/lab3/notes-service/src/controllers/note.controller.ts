...
import { CreateNoteDto, NoteDto, UpdateNoteDto } from '../dtos/note.dto';
...
export class NoteController {
  ...
  @ResponseSchema(NoteDto)
  async updateNote(@Param('id') id: number, @Body() dto: UpdateNoteDto): Promise<NoteDto> {
    try {
  ...
  @ResponseSchema(NoteDto, { isArray: true })
  async getCurrentUserNotes(@Req() request: RequestWithUser): Promise<NoteDto[]> {
    try {
      const userId = request.user.id;
      return await this.noteService.getNotesByUserId(userId);
    } catch (error) {
      console.error('Ошибка при получении заметок текущего пользователя:', error);
      throw new BadRequestError('Не удалось получить заметки текущего пользователя');
    }
  }
}
