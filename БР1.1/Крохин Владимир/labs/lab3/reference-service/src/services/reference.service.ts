...
export class ReferenceService {
  ...
  async getAllExercises(): Promise<ExerciseDto[]> {
    ...
    return exercises.map(this.mapExerciseToDto);
  }

  async getExerciseById(id: number): Promise<ExerciseDto> {
    const exercise = await dataSource.getRepository(ExerciseEntity).findOne({ where: { id } });
    if (!exercise) {
  ...
  async createExercise(dto: CreateExerciseDto): Promise<ExerciseDto> {
    ...
    const savedExercise = await dataSource.getRepository(ExerciseEntity).save(exercise);
    return this.mapExerciseToDto(savedExercise);
  }

  async updateExercise(id: number, dto: UpdateExerciseDto): Promise<ExerciseDto> {
    const exercise = await dataSource.getRepository(ExerciseEntity).findOne({ where: { id } });
    if (!exercise) {
      throw new Error('Упражнение не найдено');
    }
    Object.assign(exercise, dto);
    const updatedExercise = await dataSource.getRepository(ExerciseEntity).save(exercise);
  ...
  async getAllTargets(): Promise<TargetDto[]> {
    ...
    return targets.map(this.mapTargetToDto);
  }

  async getTargetById(id: number): Promise<TargetDto> {
    const target = await dataSource.getRepository(TargetEntity).findOne({ where: { id } });
    if (!target) {
  ...
  async createTarget(dto: CreateTargetDto): Promise<TargetDto> {
    ...
    const savedTarget = await dataSource.getRepository(TargetEntity).save(target);
  ...
  async updateTarget(id: number, dto: UpdateTargetDto): Promise<TargetDto> {
    ...
    const updatedTarget = await dataSource.getRepository(TargetEntity).save(target);
  ...
  async getAllWeaponTypes(): Promise<WeaponTypeDto[]> {
    ...
    return weaponTypes.map(this.mapWeaponTypeToDto);
  }

  async getWeaponTypeById(id: number): Promise<WeaponTypeDto> {
    const weaponType = await dataSource.getRepository(WeaponTypeEntity).findOne({ where: { id } });
    if (!weaponType) {
  ...
  async createWeaponType(dto: CreateWeaponTypeDto): Promise<WeaponTypeDto> {
    ...
    const savedWeaponType = await dataSource.getRepository(WeaponTypeEntity).save(weaponType);
    return this.mapWeaponTypeToDto(savedWeaponType);
  }

  async updateWeaponType(id: number, dto: UpdateWeaponTypeDto): Promise<WeaponTypeDto> {
    const weaponType = await dataSource.getRepository(WeaponTypeEntity).findOne({ where: { id } });
    if (!weaponType) {
      throw new Error('Тип оружия не найден');
    }
    Object.assign(weaponType, dto);
    const updatedWeaponType = await dataSource.getRepository(WeaponTypeEntity).save(weaponType);
