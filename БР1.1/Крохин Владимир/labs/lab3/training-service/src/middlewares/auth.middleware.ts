...
const authMiddleware = async (request: RequestWithUser, response: Response, next: NextFunction) => {
  ...
  if (!authorization) {
    return response.status(401).send({ message: 'Не авторизован: токен не предоставлен' });
  }
  ...
  try {
    ...
    const validateResponse = await axios.post(`${authServiceUrl}/api/auth/validate-token`, {
      token: accessToken,
    });
