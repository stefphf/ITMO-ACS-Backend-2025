'use strict';
var __decorate =
  (this && this.__decorate) ||
  function (decorators, target, key, desc) {
    var c = arguments.length,
      r =
        c < 3
          ? target
          : desc === null
            ? (desc = Object.getOwnPropertyDescriptor(target, key))
            : desc,
      d;
    if (typeof Reflect === 'object' && typeof Reflect.decorate === 'function')
      r = Reflect.decorate(decorators, target, key, desc);
    else
      for (var i = decorators.length - 1; i >= 0; i--)
        if ((d = decorators[i]))
          r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
  };
var __metadata =
  (this && this.__metadata) ||
  function (k, v) {
    if (typeof Reflect === 'object' && typeof Reflect.metadata === 'function')
      return Reflect.metadata(k, v);
  };
Object.defineProperty(exports, '__esModule', { value: true });
exports.ShotEntity = void 0;
const typeorm_1 = require('typeorm');
const series_entity_1 = require('./series.entity');
/**
 * Сущность выстрела
 */
let ShotEntity = class ShotEntity {};
exports.ShotEntity = ShotEntity;
__decorate(
  [(0, typeorm_1.PrimaryGeneratedColumn)(), __metadata('design:type', Number)],
  ShotEntity.prototype,
  'id',
  void 0,
);
__decorate(
  [
    (0, typeorm_1.Column)({ name: 'series_id' }),
    __metadata('design:type', Number),
  ],
  ShotEntity.prototype,
  'seriesId',
  void 0,
);
__decorate(
  [
    (0, typeorm_1.ManyToOne)(
      () => series_entity_1.SeriesEntity,
      series => series.shots,
    ),
    (0, typeorm_1.JoinColumn)({ name: 'series_id' }),
    __metadata('design:type', series_entity_1.SeriesEntity),
  ],
  ShotEntity.prototype,
  'series',
  void 0,
);
__decorate(
  [(0, typeorm_1.Column)(), __metadata('design:type', Number)],
  ShotEntity.prototype,
  'order',
  void 0,
);
__decorate(
  [(0, typeorm_1.Column)({ type: 'float' }), __metadata('design:type', Number)],
  ShotEntity.prototype,
  'score',
  void 0,
);
__decorate(
  [(0, typeorm_1.Column)({ type: 'float' }), __metadata('design:type', Number)],
  ShotEntity.prototype,
  'x',
  void 0,
);
__decorate(
  [(0, typeorm_1.Column)({ type: 'float' }), __metadata('design:type', Number)],
  ShotEntity.prototype,
  'y',
  void 0,
);
__decorate(
  [(0, typeorm_1.CreateDateColumn)(), __metadata('design:type', Date)],
  ShotEntity.prototype,
  'createdAt',
  void 0,
);
__decorate(
  [(0, typeorm_1.UpdateDateColumn)(), __metadata('design:type', Date)],
  ShotEntity.prototype,
  'updatedAt',
  void 0,
);
exports.ShotEntity = ShotEntity = __decorate(
  [(0, typeorm_1.Entity)('shot')],
  ShotEntity,
);
